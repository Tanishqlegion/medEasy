    ...
    return (
      <div className="flex-grow flex items-center justify-center min-h-screen">
        <div className="flex flex-col items-center gap-6 opacity-50">
          ...
          <p className="text-[11px] font-black uppercase tracking-[0.3em] text-white/40">Fetching Clinical Report...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex-grow flex items-center justify-center min-h-screen">
        <div className="flex flex-col items-center gap-6 text-center max-w-md">
          ...
          <Button onClick={() => navigate('/patient-dashboard')} className="h-12 px-8 rounded-2xl bg-white/5 border border-white/10 font-black uppercase tracking-widest text-[10px]">
    ...
    <div className="flex-grow flex flex-col px-6 py-12 text-[var(--text-main)] max-w-7xl mx-auto w-full relative z-10 bg-mesh min-h-screen">
      ...
      <motion.div variants={fadeInUp} initial="initial" animate="animate" className="flex items-center justify-between mb-10 gap-4 flex-wrap">
        <Button
          ...
          className="flex items-center gap-3 h-12 px-6 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] border-white/10 glass-panel group"
        >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-white/30">Lab Report View</p>
            <p className="text-[11px] font-black uppercase tracking-wider text-white/60">{report?.reportTitle}</p>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div variants={fadeInUp} initial="initial" animate="animate" transition={{ delay: 0.05 }}
        className="glass-panel p-10 rounded-[40px] mb-8 border-white/5 relative overflow-hidden"
      >
        <div className="absolute top-0 right-0 w-64 h-64 bg-cyan-500/5 blur-[80px]" />
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 relative z-10">
          <div>
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-white/30 mb-1">Lab Facility</p>
            <p className="text-sm font-black text-white uppercase tracking-tight">{report?.labName}</p>
          </div>
          <div>
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-white/30 mb-1">Test Type</p>
            <p className="text-sm font-black text-cyan-400 uppercase tracking-tight">{report?.testType}</p>
          </div>
          <div>
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-white/30 mb-1">Uploaded</p>
            <p className="text-sm font-black text-white uppercase tracking-tight">{new Date(report?.uploadedAt).toLocaleDateString()}</p>
          </div>
          <div>
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-white/30 mb-1">File</p>
            <p className="text-sm font-black text-white truncate">{report?.fileName}</p>
          </div>
        </div>
        {report?.notes && (
          <div className="mt-6 pt-6 border-t border-white/5 relative z-10">
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-white/30 mb-2">Lab Technician Notes</p>
            <p className="text-sm text-white/60 font-medium leading-relaxed">{report.notes}</p>
          </div>
        )}
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <motion.div variants={fadeInUp} initial="initial" animate="animate" transition={{ delay: 0.1 }}
          className="glass-panel p-8 rounded-[40px] border-white/5 flex flex-col"
        >
          <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/40 mb-6 flex items-center gap-3">
        ...
        <motion.div variants={fadeInUp} initial="initial" animate="animate" transition={{ delay: 0.15 }}
          className="glass-panel p-8 rounded-[40px] border-white/5 flex flex-col items-center justify-center gap-6 text-center"
        >
          ...
          <div>
            <h3 className="text-xl font-black uppercase tracking-tighter text-white mb-2">AI Analysis Engine</h3>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30 leading-relaxed">{report?.testType}</p>
          ...
          <p className="text-[9px] text-white/20 uppercase tracking-widest max-w-xs leading-relaxed">
      ...
      {report && (
        <motion.div
          ...
          className="glass-panel p-10 rounded-[40px] border-white/5"
        >
