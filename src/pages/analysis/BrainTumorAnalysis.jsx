  ...
  if (analysing) return (
    <div className="flex flex-col items-center justify-center py-20 gap-6">
      ...
      <div className="text-center">
        <p className="text-lg font-black uppercase tracking-tighter text-white mb-2">Brain MRI Analysis</p>
        ...
        {modelOutput && (
          <div className="mt-4 px-6 py-3 rounded-2xl border" style={{ backgroundColor: `${TUMOR_COLOR[modelOutput.prediction] || '#8b5cf6'}15`, borderColor: `${TUMOR_COLOR[modelOutput.prediction] || '#8b5cf6'}40` }}>
            ...
            <p className="text-[9px] text-white/40 uppercase mt-1">Conf: {(modelOutput.confidence * 100).toFixed(1)}%</p>
          </div>
        )}
      </div>
    </div>
    ...
    <AnimatePresence>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div className="p-8 rounded-[32px] border flex items-center gap-6"
             style={{ backgroundColor: `${tumorColor}08`, borderColor: `${tumorColor}30` }}>
          ...
          <div className="flex-grow">
            <div className="flex items-center gap-3 mb-2 flex-wrap">
              ...
              {hasTumor && (
                <span className="px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border border-rose-500/30 bg-rose-600/90 text-white animate-pulse flex items-center gap-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {modelOutput && (
            <div className="glass-panel p-6 rounded-[28px] border-white/5 space-y-3">
              ...
              <div className="p-4 rounded-[16px] border" style={{ backgroundColor: `${tumorColor}08`, borderColor: `${tumorColor}30` }}>
                <p className="text-[9px] font-black uppercase text-white/30 mb-1">Tumor Type</p>
                  ...
                  {Object.entries(modelOutput.all_confidences).map(([cls, pct]) => (
                    <div key={cls}>
                      <div className="flex justify-between mb-0.5">
                        <p className="text-[8px] font-black text-white/30 uppercase truncate max-w-[100px]">{cls}</p>
                        <p className="text-[8px] font-black text-white/30">{pct}%</p>
                      </div>
                      <div className="h-1 rounded-full bg-white/5">
                        <div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: tumorColor }} />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="p-3 rounded-[12px] bg-white/[0.03] border border-white/5 text-center">
                  <p className="text-[8px] font-black text-white/30 uppercase tracking-widest mb-1">Tumor</p>
          ...
          <div className="glass-panel p-6 rounded-[28px] border-white/5 flex flex-col items-center justify-center">
            ...
            <p className="text-[9px] font-black text-white/30 uppercase tracking-widest">Certainty</p>
          </div>
        </div>

        {/* Tumor Details */}
        {analysis.tumor_details && hasTumor && (
          <div className="glass-panel p-8 rounded-[32px]" style={{ borderColor: `${tumorColor}30`, backgroundColor: `${tumorColor}05` }}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div key={i} className="p-4 rounded-[16px] bg-white/[0.03] border border-white/5">
                  <p className="text-[8px] font-black text-white/30 uppercase tracking-widest mb-1">{item.label}</p>
                  <p className="text-[10px] font-black text-white uppercase leading-tight">{item.val || 'N/A'}</p>
                </div>
              ))}
            </div>
          </div>
